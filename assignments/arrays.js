...
//Make a copy of the original array so that it is not mutated by sorting.
//JSON parse/stringify allows for deep copying
let inventoryCopy = JSON.parse(JSON.stringify(inventory));
for (let i = 0; i < inventoryCopy.length; i++) {
  for (let j = i; j < inventoryCopy.length; j++) {
    if (
      inventoryCopy[i].car_model.toLowerCase() >
      inventoryCopy[j].car_model.toLowerCase()
    ) {
...
//Push car models into carModels from the now sorted inventoryCopy
let carModels = [];
for (let i = 0; i < inventoryCopy.length; i++) {
  carModels.push(inventoryCopy[i].car_model);
}
